...
class HarrisKCAS {
 ...
 private:
  ...
  static const std::size_t S_KCAS_BIT = 0x1;
  static const std::size_t S_RDCSS_BIT = 0x2;
  ...
  union DescriptorUnion {
    ...
    DescriptorUnion() : bits(0) {}
    DescriptorUnion(std::uintptr_t bits) : bits(bits) {}
    DescriptorUnion(RDCSSDescriptor *desc) : rdcss_descriptor(desc) {}
    DescriptorUnion(KCASDescriptor *desc) : kcas_descriptor(desc) {}
  ...
  struct RDCSSDescriptor : public RecordBase {
    ...
    DescriptorUnion before, kcas_desc;  // expected, desired
    ...
    static DescriptorUnion rdcss_read(
        const std::atomic<DescriptorUnion> *location,
        const std::memory_order memory_order, RecordHandle &found_rdcss) {
      while (true) {
  ...
  bool cas_internal(const DescriptorUnion desc, ReclaimerPin<MemReclaimer> &pin,
                    RecordHandle &found_kcas, RecordHandle &our_rdcss,
                    RecordHandle &found_rdcss, bool help = false) {
      ...
      retry:
        ...
        if (DescriptorUnion::is_kcas(value)) {
          ...
          if (value.bits != desc.bits) {
            ...
            cas_internal(value, pin, found_kcas, our_rdcss, found_rdcss, true);
            goto retry;
          } else {
 ...
 public:
  ...
  class KCASEntry {
   public:
    KCASEntry() : m_entry(DescriptorUnion{}) {}
   ...
   private:
    ...
    static std::size_t from_union(DescriptorUnion desc) {
      return (ItemBitsUnion{desc.bits}).item;
    }
    static DescriptorUnion to_union(const std::size_t &inner) {
      return DescriptorUnion{(ItemBitsUnion{inner}).raw_bits};
    }
    friend class HarrisKCAS;
   ...
   private:
    ...
    KCASDescriptor(const std::size_t descriptor_size)
        : state(0),
          ...
          m_status(KCASDescriptorStatus::UNDECIDED),
          m_num_entries(0),
          m_descriptors(reinterpret_cast<EntryPayload *>(this + 1)) {}
  ...
  HarrisKCAS(__attribute__((unused)) const std::size_t threads,
  ...
  KCASDescriptor *create_descriptor(
    ...
    KCASDescriptor *desc = new (m_reclaimer->malloc(
        sizeof(KCASDescriptor) + sizeof(EntryPayload) * descriptor_size))
  ...
  bool cas([[maybe_unused]] const std::size_t thread_id,
           ReclaimerPin<MemReclaimer> &pin, KCASDescriptor *desc) {
    ...
    our_kcas.set(desc);
    bool res = cas_internal(DescriptorUnion::from_kcas(desc), pin, found_kcas,
                            our_rdcss, found_rdcss);
    desc->state = 1;
    pin.retire(our_kcas);
    return res;
  }
  ...
  ValType read_value(
      [[maybe_unused]] const std::size_t thread_id,
      ReclaimerPin<MemReclaimer> &pin, const KCASEntry<ValType> *location,
      const std::memory_order memory_order = std::memory_order_seq_cst) {
    static_assert(
        !std::is_pointer<ValType>::value and !std::is_reference<ValType>::value,
        "Type is not a value.");
    ...
    while (true) {
      ...
      if (DescriptorUnion::is_kcas(desc)) {
        cas_internal(desc, pin, found_kcas, our_rdcss, found_rdcss, true);
        continue;
      }
  ...
  PtrType read_ptr(
      [[maybe_unused]] const std::size_t thread_id,
      ReclaimerPin<MemReclaimer> &pin, const KCASEntry<PtrType> *location,
      const std::memory_order memory_order = std::memory_order_seq_cst) {
